...
class EmployeeClass: #we are make class here
    def __init__(self,root):#this is defaule constructor. root is the object
        ...
        cmd_search=ttk.Combobox(SearchFrame,textvariable=self.var_searchby,values=("Select","Email","Name","Contact"),font=("goudy old style",15),state='readonly',justify=CENTER)
        ...
        cmd_utype=ttk.Combobox(self.root,textvariable=self.var_utype,values=("Admin","Employee"),font=("goudy old style",15),state='readonly',justify=CENTER)
        ...
        btn_add=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),cursor="hand2",bg="#2196f3",fg="white").place(x=500,y=305,width=110,height=28)
        btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),cursor="hand2",bg="#4caf50",fg="white").place(x=620,y=305,width=110,height=28)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),cursor="hand2",bg="#f44336",fg="white").place(x=740,y=305,width=110,height=28)
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),cursor="hand2",bg="#607d8b",fg="white").place(x=860,y=305,width=110,height=28)
